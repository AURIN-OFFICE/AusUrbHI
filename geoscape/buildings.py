...
class StudyAreaProcessor:
    ...
    def compute_fields(self):
        ...
        self.results = [
            {
                'SA1_CODE21': sa1_code,
                'SP_ADJ': group_data['SP_ADJ'].str.contains('Yes', case=False, na=False).sum() / len(group_data),
                'ROOF_HGT': group_data['ROOF_HGT'].mean(),
                'MAT_Tile': group_data['PR_RF_MAT'].value_counts().get('Tile', 0) / len(group_data),
                'MAT_Metal': group_data['PR_RF_MAT'].value_counts().get('Metal', 0) / len(group_data),
                'MAT_Concre': group_data['PR_RF_MAT'].value_counts().get('Flat Concrete', 0) / len(group_data),
                'ROOF_CLR': group_data['ROOF_CLR'].apply(self.compute_luminance).mean(),
