...
# -*- coding: utf-8 -*-
"""
run_tests.py tests the accuracy of ehfheatwaves.py compared to heatwave boundary_data
...
class TestIdentifyHW(unittest.TestCase):
    ...
    # Some example EHF index boundary_data
...
class TestGetOptions(unittest.TestCase):
    ...
    def testNoFilesError(self):
        """Should return an exception if no boundary_data files are provided."""
...
if __name__=='__main__':
    ...
    # Test the script as a whole against climpact2 boundary_data.
    ...
    # Load the comparison boundary_data
    chwdata = np.load('hwdata.npy')
    for i in range(0,6):
        # The old boundary_data is upside down
        thwdata[i,...] = np.fliplr(thwdata[i,...])

    # Take the difference between the test and comparison boundary_data
