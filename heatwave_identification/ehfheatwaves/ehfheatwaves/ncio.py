...
class TimeData(object):
    """Data class for time boundary_data from an netcdf file."""
...
def get_mask(options)->np.ndarray:
    """get_mask loads the land-sea mask boundary_data from a NetCDF4 file."""
...
def load_bp_data(options, timedata:TimeData, variable:str='tmax', mask:np.ndarray=None)->np.ndarray:
    """Load the tmax or tmin boundary_data for the base period.
    ...
    - temp : Temperature boundary_data array.
    """
    # Determine if we need tmax or tmin and whether the boundary_data are in multiple files.
    if variable=='tmax':
    ...
    try:
        ...
        raise IndexError(
                ...
                "Please make sure your input file is daily frequency boundary_data and/or that it contains "
                f"the base period. You wanted {options.bpstart}-{options.bpend} and your boundary_data have "
...
def get_all_data(files:str, vname:str, options)->tuple:
    """Load all temperature boundary_data from a netcdf file.
    ...
    - temp : boundary_data in (time, x, y) coordinates.
    - lats : latitudes
    """
    if any([(wildcard in files) for wildcard in ['*','?','[']]):
        tempnc = nc.MFDataset(files, 'r')
...
def save_yearly(
    ...
    """Save yearly boundary_data to netcdf file.
...
def save_daily(
    ...
    """save_daily saves the daily boundary_data to netcdf file. Input arrays are 2D, (time, space) and are
    ...
    - options :
    - timedata : TimeData object.
    - original_shape : The original shape of the input boundary_data.
    - mask : Land-sea mask.
...
def save_ehi(
    ...
    """Save the daily boundary_data to netcdf file. Input arrays are 2D, (time, space) and are either
