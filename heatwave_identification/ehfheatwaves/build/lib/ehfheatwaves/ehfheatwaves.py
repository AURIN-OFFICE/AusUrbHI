...
- parse arguments
- load the base period boundary_data
- calculate the percentile thresholds
- load the remaining boundary_data
...
def window_percentile(temp:np.ndarray, daysinyear:int=365, wsize:int=15)->np.ndarray:
    ...
    - temp : Temperature boundary_data.
    ...
    # Remaining nans are missing boundary_data.
...
def split_hemispheres(EHF:np.ndarray, north:bool, south:bool)->tuple:
    """Split the input boundary_data by hemispheres, and glue them back together after heatwave
    calculations.

    The EHF spatial axes are reshaped into a single dimension.
    The output arrays are 2D. When saving, boundary_data should be reshaped or indexed with a land-sea mask.
...
def main():
    ...
    # Load time boundary_data
    if options.verbose: print("Loading boundary_data")
    if options.tmaxfile:
    ...
    # Load the temperature boundary_data over the base period
    ...
    # Remove leap days from boundary_data
    ...
    # Save yearly boundary_data to netcdf
    if options.yearlyout:
        if not options.noehf:
            ncio.save_yearly(
                    HWA_EHF,
    ...
    # Save daily boundary_data to netcdf
    if options.dailyout:
        if options.keeptave:
            ncio.save_daily(
                    EHF,
