...
class HeatNetCDFProcessor:
    ...
    def refine_and_buffer_netcdf(self, buff: bool = True) -> None:
        ...
        for m in tqdm(('max', 'min'), desc="Refining NetCDF files", total=2):
            netcdf = xr.open_dataset(self.silo_folder + f'2010_2022_{m}_temp.nc')
            ...
            clipped.to_netcdf(self.silo_folder + f'2010_2022_{m}_temp_clipped.nc')

    @staticmethod
    def run_ehfheatwaves_package() -> None:
        """Run the ehfheatwaves analysis package on the NetCDF files
        """
        cwd = os.getcwd()
        os.chdir('../_data/AusUrbHI HVI data unprocessed/Longpaddock SILO LST/')
        command = [
            "ehfheatwaves",
