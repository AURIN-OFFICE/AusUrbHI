...
for m in ('max', 'min'):
    ...
    netcdf = xr.open_dataset(f'../_data/longpaddock_silo_lst/{m}/2016_2021_{m}_temp.nc')
    ...
    clipped.to_netcdf(f'../_data/longpaddock_silo_lst/2016_2021_{m}_temp_clipped_unbuffered.nc')

    # buffered version
    shapefile_buffered = shapefile.buffer(0.05)
    clipped = netcdf.rio.clip(shapefile_buffered.geometry.apply(mapping), shapefile.crs)
    clipped.to_netcdf(f'../_data/longpaddock_silo_lst/2016_2021_{m}_temp_clipped_buffered.nc')
